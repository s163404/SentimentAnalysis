...
numb_key = 1   # twitter api のoauth認証(set_Oauth)に関する情報の 配列番号
numb_split = 1  #  収集データを並列的に収集可能にするため、この指定数で分割し、numb_keyの部分だけを収集する
#filename = "extra20180710.txt"
filename = "followerlist_test.txt"  # 入力ファイル
output_filename = "followerlist_test-"+str(numb_key)+".txt" # 出力ファイル
...
def set_Oauth(numb_key):
    ...
    CK_array = [
        #  Consumer Key を配列で
        "bwYmFcRYeLohd6dXzmWmNNwzN",
        "A5ybzADocuCK49xsSc2qk5bpD"

    ]

    CS_array = [
        #  Consumer Secret を配列で
        "KZIMIM9W7D1m7GQPqj3vNMXc779DZjhEQKO9pKD7207V6hSUUM",
        "ZMbFdY9pkOz9E3M7aWynlCYnoMgGDyZbvftMOcUVSg6CRE11P6"
    ]

    AT_array = [
        # access token を配列で
        "414823726-QylltNtoQ0rDR7JIfcglU7d6eSemAqOHI7DLdZ9u",
        "414823726-DRlYROh6kHiXxNENsGzNm07KoTlUaSDkqrvUAVxn"
    ]

    AS_array = [
        # access token secret を配列で
        "BNVsZ63FdXJfu6cTSEi1pVs7cMREtXDX3OxpqUyBZliNE",
        "CvRN6h4vaC5Wg0tazvfK0AJt5pXPcoIPIgfGyChQ20FVg"
    ]
